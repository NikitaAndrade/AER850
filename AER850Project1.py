...
# output_file = open('output_posttuning.txt', 'w')
output_file = open('output_pretuning.txt', 'w')
...
test_x = strat_test_set.drop(columns = ["Step"])
...
my_scaler.fit(test_x.iloc[:,:])
scaled_data = my_scaler.transform(test_x.iloc[:,:])
scaled_data_df = pd.DataFrame(scaled_data, columns=test_x.columns[:])
test_x = scaled_data_df
...
model1_prediction = model1.predict(test_x)
from sklearn.metrics import mean_absolute_error
model1_train_mae = mean_absolute_error(model1_prediction, test_y)
...
# model2 = RandomForestRegressor(max_depth = None, max_features = 'sqrt', min_samples_leaf= 1, min_samples_split= 2, n_estimators=10, random_state=16)
model2.fit(train_x, train_y)
model2_predictions = model2.predict(test_x)
model2_train_mae = mean_absolute_error(model2_predictions, test_y)
print("Model 2 training MAE is: ", round(model2_train_mae,2))


#Model 3: Decision Tree Regressor
model3= DecisionTreeRegressor() #Pre Grid Search
# model3 = DecisionTreeRegressor(criterion='absolute_error', max_depth=10, max_features='sqrt', max_leaf_nodes=20, min_impurity_decrease=0.0, min_samples_leaf=2, min_samples_split=10, random_state=16)
model3.fit(train_x, train_y)
model3_predictions = model3.predict(test_x)
model3_train_mae = mean_absolute_error(model3_predictions, test_y)
...
#Model 1
##Scores
accuracym1 = accuracy_score(test_y.iloc[:, 0], model1_prediction.iloc[:, 0])
precisionm1 = precision_score(test_y.iloc[:, 0], model1_prediction.iloc[:, 0], average= 'weighted')
f1m1 = f1_score(test_y.iloc[:, 0], model1_prediction.iloc[:, 0], average= 'weighted')
...
##Confusion Matrix 
cm1 = confusion_matrix(test_y.iloc[:, 0], model1_prediction.iloc[:, 0])
...
#Model 2
##Scores
accuracym2 = accuracy_score(test_y.iloc[:, 0], model2_predictions.iloc[:, 0])
precisionm2 = precision_score(test_y.iloc[:, 0], model2_predictions.iloc[:, 0], average= 'weighted')
f1m2 = f1_score(test_y.iloc[:, 0], model2_predictions.iloc[:, 0], average= 'weighted')
...
##Confusion Matrix 
cm2 = confusion_matrix(test_y.iloc[:, 0], model2_predictions.iloc[:, 0])
class_labels = [str(i) for i in range(1, 14)]
...
#Model 3
##Scores
accuracym3 = accuracy_score(test_y.iloc[:, 0], model3_predictions.iloc[:, 0])
precisionm3 = precision_score(test_y.iloc[:, 0], model3_predictions.iloc[:, 0], average= 'weighted')
f1m3 = f1_score(test_y.iloc[:, 0], model3_predictions.iloc[:, 0], average= 'weighted')
...
##Confusion Matrix 
cm3 = confusion_matrix(test_y.iloc[:, 0], model3_predictions.iloc[:, 0])
class_labels = [str(i) for i in range(1, 14)]
